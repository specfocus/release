 ...
 */
const useSuggestions = ({ allowCreate, allowDuplicates, allowEmpty, choices, createText = 'ra.action.create', createValue = '@@create', emptyText = '', emptyValue = null, limitChoicesToValue, matchSuggestion, optionText, optionValue, selectedItem, suggestionLimit = 0, translateChoice, }) => {
    const translate = (0, i18n_1.useTranslate)();
    const { getChoiceText, getChoiceValue } = (0, useChoices_1.default)({
        optionText,
        optionValue,
        translateChoice,
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
    const getSuggestions = (0, react_1.useCallback)((0, exports.getSuggestionsFactory)({
        allowCreate,
        allowDuplicates,
        allowEmpty,
        choices,
...
const defaultMatchSuggestion = getChoiceText => (filter, suggestion, exact = false) => {
    ...
    const isReactElement = (0, react_1.isValidElement)(suggestionText);
 ...
 * // Will return [{ id: 2, name: 'publisher' }]
 */
const getSuggestionsFactory = ({ allowCreate = false, allowDuplicates = false, allowEmpty = false, choices = [], createText = 'ra.action.create', createValue = '@@create', emptyText = '', emptyValue = null, optionText = 'name', optionValue = 'id', getChoiceText, getChoiceValue, limitChoicesToValue = false, matchSuggestion = defaultMatchSuggestion(getChoiceText), selectedItem, suggestionLimit = 0, }) => filter => {
    let suggestions = [];
 ...
 */
const removeAlreadySelectedSuggestions = (suggestions, selectedItems, getChoiceValue) => {
    if (!selectedItems) {
        return suggestions;
    }
    ...
    return suggestions.filter(suggestion => !selectedValues.includes(getChoiceValue(suggestion)));
};
/**
 * @example
 *
